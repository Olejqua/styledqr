...
import type { PrettyQROptions, StyleOptions } from '../encoder/types'
...
export class SVGRenderer {
  ...
  private renderQRCode(
    ...
    const analyzer = new NeighborAnalyzer(modules)
    ...
    if (style.gradient) {
      ...
      elements.push(GradientRenderer.createGradient({
        ...style.gradient,
        id: gradientId
      }))
    }
    ...
    for (let row = 0; row < qrSize; row++) {
      for (let col = 0; col < qrSize; col++) {
        ...
        if (this.options.logo && LogoRenderer.shouldClearForLogo(row, col, this.options.logo, qrSize)) {
          continue
        }
