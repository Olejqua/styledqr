...
export class ShapeRenderer {
  ...
  static render(options: ShapeOptions): string {
    ...
    switch (style) {
      case 'square':
        return this.square(size, cornerRadius)
      case 'rounded':
        return this.square(size, cornerRadius || size * 0.2)
      case 'circle':
   ...
   */
  static createGradient(id: string, type: 'linear' | 'radial', colors: string[], direction: number = 0): string {
    if (type === 'linear') {
